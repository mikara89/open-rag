...
| Reprocess | AuthenticatedUser / `ICurrentTenant` | Tenant-scoped tracking document/version and tenant-scoped chunk/embedding deletes | Source/Markdown keys validated before event publication | Generic 404; invalid state 409; denial makes no event/storage call |
...
| Worker chunking | Command tenant | Scoped version/document/run/step/chunk/embedding operations; full identities checked | Markdown/JSON keys checked before storage; chunks use command tenant | Foreign scope no-op; no storage/chunker/persistence/event |
...
| Worker embeddings | Command tenant | Scoped run/document/version/chunks/embeddings; every chunk identity checked | Only validated chunks reach embedding provider | Foreign scope no-op; no provider/persistence/event |
